...
uintmax_t Myls::dirSize(directory_entry &dir) {
    uintmax_t size = 0;
...
string Myls::get_time(directory_entry &item) {
...
string Myls::get_filename(directory_entry& item) {
    return item.path().filename().string();
}

bool Myls::sortSize(directory_entry i1, directory_entry i2)
{
...
bool Myls::sortTime(directory_entry i1, directory_entry i2)
{
    return (last_write_time(i1) < last_write_time(i2));
}

bool Myls::sortName(directory_entry i1, directory_entry i2)
{
    return (i1.path().stem().string() < i2.path().stem().string());
}

bool Myls::sortExtension(directory_entry i1, directory_entry i2)
{
    return (i1.path().extension() < i2.path().extension());
}

int Myls::list_items_in_dir(path p, ProgramOptions& programOptions) {
